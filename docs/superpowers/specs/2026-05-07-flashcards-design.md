# Spec — Flashcard + Audio Drill System
...
Produce two artifacts from the same source-of-truth markdown:
...
Initial scope: lessons 1-22, organized into 8 topical bundles. Estimated ~2000-2500 cards.
...
```
                    ┌──────────────────────────────┐
                    │ lesson_NN/cards.yml          │  (per-lesson primary + extended)
                    │ cards_topical/topic_*.yml    │  (multi-lesson)
                    └──────────────┬───────────────┘
                                   │
                       ┌───────────┴───────────┐
                       │                       │
              ...
              └────────┬─────────┘    └────────┬─────────┘
                       │                       │
                       ▼                       ▼
              dist/transferencia.apkg   audio/lesson_NN.mp3
              dist/cards.json           (cumulative)
              (for Phase 2)
```

Both generators read every `lesson_NN/cards.yml` and `cards_topical/topic_*.yml`. They share the parser and validator under `build/lib/`.
...
Standard targets: `install`, `validate`, `anki`, `cards-json`, `audio`, `audio-quick`, `all`, `clean`. See [`docs/learning-system.md`](../../learning-system.md) for the contents.
...
- `make audio` produces all 22 tracks
...
M1-M3 form the first usable artifact: ~150 cards plus the generator. Ship that and study before continuing.
